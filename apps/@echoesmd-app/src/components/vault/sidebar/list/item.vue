<script setup lang="ts">
  ...
  const createTippy = (button: HTMLElement) => {
      ...
      if (props.file.type === 'page') {
        ...
        for (const group of groups) {
          ...
          for (let i = 0; i < tabs.length; i++) {
            const tab = tabs[i];
            if (tab.id === props.file.id) {
              echoes.removeTab(tab, group.id);
            }
          }
        }
      } else {
        const tabs = instance.getFilesUnderItem(props.file);
        const groups = echoes.getGroups();
        for (const group of groups) {
          for (const tab of tabs) {
            const index = group.tabs.findIndex((t) => t.id === tab.id);
            if (index !== -1) {
              echoes.removeTab(group.tabs[index].id, group.id);
            }
          }
        }
      }
    ...
    editButton.setAttribute("title", "Rename");
    ...
    <echoes-ui-container hover class="flex justify-between cursor-pointer p-1 rounded w-full text-xs font-normal" @click.stop="handleItemClick">
        <div class="flex items-center gap-x-1">
          <div class="text-neutral-500">
            <svg v-if="file.type === 'folder'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="size-4" :class="{'rotate-90': openDropdown}">
          ...
          <form v-if="edit" @submit.stop.prevent="handleSubmit">
            <input :class="background.container" v-model="name" @click.stop @blur="handleSubmit" :id="`input-${file.id}`" class="w-full" />
          </form>
